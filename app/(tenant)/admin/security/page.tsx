...
// ── 상수 ────────────────────────────────────────────────────────
const SEC_SEVERITY_STYLE: Record<string, string> = {
...
const RANSOM_SEVERITY_STYLE: Record<string, string> = {
...
export default function SecurityMonitoringPage() {
  // 최상위 탭
  const [pageTab, setPageTab] = useState<'security' | 'ransomware'>('security');
  ...
  const loadSecData = useCallback(async () => {
    ...
    try {
      ...
      showToast('보안 데이터 로드 실패', false);
    } finally {
      setSecLoading(false);
    }
  }, [hours, typeFilter]);

  useEffect(() => { loadSecData(); }, [loadSecData]);
  useEffect(() => {
    const timer = setInterval(() => { loadSecData(); }, 30_000);
  ...
  const hourlyData = Array.from({ length: 24 }, (_, i) => {
    const cnt = events.filter(ev => new Date(ev.createdAt).getHours() === i).length;
    return { hour: `${String(i).padStart(2, '0')}시`, count: cnt };
  });

  // ── 랜섬웨어 상태 ──────────────────────────────────────────────
  const [ransomAlerts,  setRansomAlerts]  = useState<RansomwareAlert[]>([]);
  const [backups,       setBackups]       = useState<BackupRecord[]>([]);
  const [ransomLoading, setRansomLoading] = useState(false);
  const [updating,      setUpdating]      = useState<string | null>(null);
  const [ransomTab,     setRansomTab]     = useState<'alerts' | 'backups' | 'stats'>('alerts');
  const [ransomExpanded, setRansomExpanded] = useState<string | null>(null);
  const [statusFilter,  setStatusFilter]  = useState('');
  const [sevFilter,     setSevFilter]     = useState('');

  const loadRansomData = useCallback(async () => {
    setRansomLoading(true);
    try {
      ...
      setRansomAlerts(alertsRes.data ?? []);
      setBackups(backupsRes.data ?? []);
    } catch {
      showToast('랜섬웨어 데이터 로드 실패', false);
    } finally {
      setRansomLoading(false);
    }
  }, [statusFilter, sevFilter]);

  // 랜섬웨어 탭 진입 시 데이터 로드
  useEffect(() => {
    if (pageTab === 'ransomware') loadRansomData();
  }, [pageTab, loadRansomData]);
  ...
  const openAlerts     = ransomAlerts.filter(a => a.status === 'open').length;
  const criticalAlerts = ransomAlerts.filter(a => a.severity === 'CRITICAL').length;
  ...
  const ransomTypeCount = ransomAlerts.reduce<Record<string, number>>((acc, a) => {
    const t = ALERT_TYPE_KO[a.alertType] ?? a.alertType;
    acc[t] = (acc[t] ?? 0) + 1;
    return acc;
  }, {});
  const typeChartData = Object.entries(ransomTypeCount).map(([name, value]) => ({ name, value }))
    .sort((a, b) => b.value - a.value);
  const sevCount = ransomAlerts.reduce<Record<string, number>>((acc, a) => {
  ...
  const trendData = Array.from({ length: 7 }, (_, i) => {
    ...
    const count = ransomAlerts.filter(a => a.createdAt.startsWith(dateStr)).length;
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">보안 이벤트 · IP 차단 · 랜섬웨어 대응 통합 관리</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={pageTab === 'security' ? loadSecData : loadRansomData}
            disabled={pageTab === 'security' ? secLoading : ransomLoading}
            ...
            <RefreshCw className={`w-4 h-4 ${(pageTab === 'security' ? secLoading : ransomLoading) ? 'animate-spin' : ''}`} />
            새로고침
          </button>
        </div>
      </div>
      ...
      {/* 최상위 탭 */}
      <div className="flex gap-1 bg-gray-800/60 rounded-xl p-1 w-fit border border-gray-700/50">
        <button
          onClick={() => setPageTab('security')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${pageTab === 'security' ? 'bg-cyan-700 text-white' : 'text-gray-400 hover:text-white'}`}>
          <Shield className="w-4 h-4" />보안 이벤트
        </button>
        <button
          onClick={() => setPageTab('ransomware')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${pageTab === 'ransomware' ? 'bg-red-700 text-white' : 'text-gray-400 hover:text-white'}`}>
          <AlertTriangle className="w-4 h-4" />랜섬웨어 대응
          {openAlerts > 0 && (
            <span className="bg-red-500 text-white text-xs rounded-full px-1.5 py-0.5 leading-none">{openAlerts}</span>
          )}
        </button>
      </div>
                        ...
                        return (
                          <div key={ev.id} className="hover:bg-gray-800/30 transition-colors">
                            <div className="flex items-center gap-3 px-4 py-2.5 cursor-pointer"
                              ...
                              <span className={`text-xs px-2 py-0.5 rounded font-medium whitespace-nowrap ${SEC_SEVERITY_STYLE[sev] ?? SEC_SEVERITY_STYLE.LOW}`}>
                                {sev}
                              </span>
      ...
      {pageTab === 'ransomware' && (
        <>
          ...
          <div className="flex gap-1 bg-gray-800/50 rounded-lg p-1 w-fit">
            {([
              { id: 'alerts',  label: `알림 목록 (${ransomAlerts.length})`,  icon: <AlertTriangle className="w-3.5 h-3.5" /> },
              ...
              <button key={tab.id} onClick={() => setRansomTab(tab.id)}
                className={`flex items-center gap-1.5 px-4 py-1.5 rounded-md text-sm transition-colors ${ransomTab === tab.id ? 'bg-gray-600 text-white' : 'text-gray-400 hover:text-white'}`}>
                {tab.icon}{tab.label}
              </button>
            ))}
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                <div className="px-5 py-3 border-b border-gray-800 flex items-center gap-2">
                  ...
                  <span className="ml-auto text-xs text-gray-500">{ransomAlerts.length}건</span>
                </div>
                {ransomLoading ? (
                  <div className="flex items-center justify-center h-32 text-gray-400">
                    <RefreshCw className="w-5 h-5 animate-spin mr-2" />로딩 중...
                  </div>
                ) : ransomAlerts.length === 0 ? (
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {ransomAlerts.map(alert => {
                          const isExp = ransomExpanded === alert.id;
                          return (
                            <>
                              <tr key={alert.id}
                                className="border-b border-gray-800/50 hover:bg-gray-800/20 transition-colors cursor-pointer"
                                onClick={() => setRansomExpanded(isExp ? null : alert.id)}>
                                <td className="px-4 py-3">
                                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${RANSOM_SEVERITY_STYLE[alert.severity] ?? ''}`}>
                                    {alert.severity}
                                  </span>
                                </td>
                                ...
                                <td className="px-4 py-3">
                                  <div className="flex items-center gap-1.5" onClick={e => e.stopPropagation()}>
                                    {alert.status === 'open' && (
                                      <>
                                        <ActionBtn label="조사 중" onClick={() => updateRansomStatus(alert.id, 'investigating')}
                                    ...
                                    {['investigating', 'contained'].includes(alert.status) && (
                                      <>
                                        <ActionBtn label="해결됨" onClick={() => updateRansomStatus(alert.id, 'resolved')}
                                          disabled={updating === alert.id} color="green" />
                                        <ActionBtn label="오탐" onClick={() => updateRansomStatus(alert.id, 'false_positive')}
